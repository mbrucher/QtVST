...
"}";
  ...
  update_gain(simple_eq->getParameter(0));
}

void QSimpleEQ::update_gain(int value)
{
  simple_eq->setParameter(0, std::pow(10, value / 200.));
}

void QSimpleEQ::update_gain(float value)
{
  int intValue = static_cast<int>(std::log(value) / std::log(10.f) * 200);
  gain_slider->setValue(intValue);  
}

void QSimpleEQ::update_oversampling_log(int value)
{
  simple_eq->set_oversampling(1 << (value + 1));
}

void QSimpleEQ::update_oversampling(int value)
{
  int oversampling = -2;
  while(value != 0)
  {
